    ...
    <section id="experience" className="min-h-screen flex items-center justify-center px-6 py-20">
      <div className="mx-auto max-w-4xl w-full">
        ...
        <div className="relative pl-8 md:pl-12">
          {/* Green timeline rail */}
          <span className="absolute left-[7px] top-3 bottom-3 w-px bg-brand/30" aria-hidden="true" />
          {experience.map((job, index) => (
            <div key={index} className="relative mb-8 last:mb-0">
              {/* Timeline node */}
              <span className="absolute -left-8 md:-left-12 top-8 flex size-4 items-center justify-center" aria-hidden="true">
                <span className="absolute size-4 rounded-full bg-brand/15" />
                <span className="relative size-2 rounded-full bg-brand ring-2 ring-brand/20" />
              </span>
              <motion.div
